...
const ReportResultsTable: React.FC<ReportResultsTableProps> = ({ data, columns, tableDataLoaded }) => {
                  ...
                  <th key={header.id} colSpan={header.colSpan} className={classNames('bg-gray-50', {'rounded-tl-lg': index === 0}, {'rounded-tr-lg': index === array.length-1})}>
                    {header.isPlaceholder ? null : (
                      ...
                      >
                        ...
                        <ColumnSortIndicator sort={header.column.getIsSorted() as string} className="ml-1 justify-center"/>
                      </div>
                    )}
                  </th>
                )
